...
async function makeCircleWithMembers(memberCount) {
  const creator = await makeStudent('Creator');
  const { circle, inviteToken } = await circlesService.createCircle('Test Crew', creator.id);
  ...
  return { circle, creator, members };
}

describe('Circle pools service', () => {
  ...
  test('createPool (private) inserts pool with outcomes', async () => {
    const { circle, creator } = await makeCircleWithMembers(3);
  ...
  test('createPool rejects kickoff in the past', async () => {
    const { circle, creator } = await makeCircleWithMembers(2);
  ...
  test('predictInPool deducts points atomically and inserts prediction', async () => {
    const { circle, creator, members } = await makeCircleWithMembers(3);
    const pool = await poolsService.createPool(circle.id, creator.id, {
      poolType: 'private', title: 'x', outcomeA: 'A', outcomeB: 'B',
    ...
    const pred = await db('circle_pool_predictions').where({ pool_id: pool.id, student_id: members[1].id }).first();
  ...
  test('predictInPool rejects insufficient points', async () => {
    ...
    await circlesService.joinCircleByToken(inviteToken, poorMember.id);
    const pool = await poolsService.createPool(circle.id, creator.id, {
      poolType: 'private', title: 'x', outcomeA: 'A', outcomeB: 'B',
      kickoffAt: new Date(Date.now() + 3600000), stakeAmount: 50,
    });
  ...
  test('predictInPool rejects non-member', async () => {
    ...
    await expect(poolsService.predictInPool(pool.id, { studentId: outsider.id }, 'A'))
      .rejects.toMatchObject({ code: 'NOT_CIRCLE_MEMBER' });
  });
  ...
  test('closeExpiredPools marks past-kickoff pools as closed', async () => {
    ...
    const after = await db('circle_pools').where({ id: pool.id }).first();
