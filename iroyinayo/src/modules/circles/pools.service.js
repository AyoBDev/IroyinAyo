...
async function createPool(circleId, creatorId, opts) {
  ...
  const member = await db('circle_members').where({ circle_id: circleId, student_id: creatorId }).first();
  if (!member) throw err('NOT_CIRCLE_MEMBER', 'Not a member', 'You\'re not a member of this circle.', 403);
...
async function predictInPool(poolId, actor, outcome) {
  ...
  const result = await db.transaction(async (trx) => {
    ...
    if (studentId) {
      ...
      if (!member) throw err('NOT_CIRCLE_MEMBER', 'Not a member', 'You\'re not a member of this circle.', 403);
      const existing = await trx('circle_pool_predictions').where({ pool_id: poolId, student_id: studentId }).first();
      ...
      const [prediction] = await trx('circle_pool_predictions').insert({
        pool_id: poolId, student_id: studentId, predicted_outcome: outcome, points_locked: pool.stake_amount,
      }).returning('*');
      return { prediction, circleId: pool.circle_id };
    } else {
      const existing = await trx('circle_pool_predictions').where({ pool_id: poolId, pending_account_id: pendingAccountId }).first();
  ...
  const io = getIO();
  if (io) {
    io.to(`circle:${result.circleId}`).emit('circle:pool:prediction', { poolId, predictionId: result.prediction.id });
  }
...
async function closeExpiredPools() {
  const result = await db('circle_pools')
    .where('status', 'open')
...
async function listPoolsForCircle(circleId) {
  return db('circle_pools')
    .where({ circle_id: circleId })
    .orderBy('kickoff_at', 'desc');
}

async function getPoolDetail(poolId, studentId) {
  const pool = await db('circle_pools').where({ id: poolId }).first();
  if (!pool) throw err('POOL_NOT_FOUND', 'No pool', 'Pool not found.', 404);
  const member = await db('circle_members').where({ circle_id: pool.circle_id, student_id: studentId }).first();
  if (!member) throw err('NOT_CIRCLE_MEMBER', 'Not a member', 'You\'re not a member of this circle.', 403);
  const predictions = await db('circle_pool_predictions')
    .leftJoin('students', 'students.id', 'circle_pool_predictions.student_id')
    .where({ pool_id: poolId })
    .select('circle_pool_predictions.id', 'circle_pool_predictions.student_id', 'circle_pool_predictions.predicted_outcome', 'circle_pool_predictions.payout', 'students.name');
...
module.exports = { STAKE_MIN, STAKE_MAX, createPool, predictInPool, closeExpiredPools, listPoolsForCircle, getPoolDetail };
