...
async function resolveMarket(marketId, winningOutcomeId) {
  ...
  // Cascade resolution to any circle pools wrapping this market. Public circle
  ...
  try {
    ...
    for (const pool of wrappingPools) {
      try {
        ...
        console.error(`[multi_markets] auto-resolve circle pool ${pool.id} failed:`, e.message);
      }
    }
  } catch (e) {
    // Don't let a circle-side issue affect the market-resolved return value.
    console.error('[multi_markets] circle pool cascade failed:', e.message);
  }
