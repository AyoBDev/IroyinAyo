...
export default function Circles() {
  ...
  if (loading) return <div className="p-4 text-ink-muted">Loading circles…</div>;
    ...
    return (
      <div className="p-4 max-w-[640px] mx-auto">
        <div className="bg-paper border border-line rounded-2xl p-8 text-center mt-12">
          ...
          <h2 className="font-serif text-section mb-2">No circles yet</h2>
          <p className="text-ink-muted text-body-sm mb-6">Create a circle with your friends and predict together.</p>
          <button onClick={() => setShowCreate(true)} className="px-5 py-3 rounded-xl bg-emerald text-white font-medium">
            Create your first circle
          </button>
        </div>
        {showCreate && <CreateCircleModal onClose={() => setShowCreate(false)} onCreated={(circle) => navigate(`/circles/${circle.id}`)} />}
      </div>
    );
  }
  ...
  return (
    <div className="p-4 max-w-[640px] mx-auto">
      <div className="flex justify-between items-center mb-4">
        <h2 className="font-serif text-section">Your Circles</h2>
      ...
      <div className="flex flex-col gap-2">
        {circles.map((c) => (
          <button key={c.id} onClick={() => navigate(`/circles/${c.id}`)} className="bg-paper border border-line rounded-xl p-4 text-left">
