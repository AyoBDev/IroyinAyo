...
export default function JoinCrewLanding() {
  ...
  useEffect(() => {
    fetch(`/api/crews/invites/${token}/preview`)
      .then(async (r) => {
  ...
  async function handleJoin() {
    ...
    setJoining(true);
    try {
      await apiFetch(`/api/crews/invites/${token}/join`, { method: 'POST' });
      navigate(`/crews/${preview.crewId}`);
    } catch (e) {
  ...
  if (error) return (
    <div className="p-4 max-w-[420px] mx-auto text-center mt-12">
      ...
      <button onClick={() => navigate('/crews')} className="px-4 py-2 rounded-lg bg-paper border border-line text-[13px]">Go to my crews</button>
    </div>
  );
  ...
  return (
    <div className="p-4 max-w-[420px] mx-auto text-center mt-12">
      ...
      <p className="text-[13px] text-ink-muted mb-6">{preview.memberCount} member{preview.memberCount === 1 ? '' : 's'} in this crew{preview.isFull ? ' — it\'s currently full.' : '.'}</p>
      <button onClick={handleJoin} disabled={joining || preview.isFull} className="w-full py-3 rounded-xl bg-emerald text-white font-medium disabled:opacity-60">
        {joining ? 'Joining…' : preview.isFull ? 'Crew is full' : (user ? 'Join crew' : 'Sign in to join')}
      </button>
    </div>
  );
}
